...
class ProductAttribute
{
    ...
    {
        $stmt = $this->db->prepare('SELECT * FROM product_attribute_values WHERE product_id = ?');
        if (!$stmt) {
    ...
    public function create(string $productId, string $attributeName, string $attributeValue): ?array
    {
        try {
            v::stringType()->notEmpty()->assert($productId);
            v::stringType()->notEmpty()->assert($attributeName);
            v::stringType()->notEmpty()->assert($attributeValue);
        ...
        $stmt->bind_param('sss', $productId, $attributeName, $attributeValue);
    ...
    {
        $stmt = $this->db->prepare('SELECT * FROM product_attribute_values WHERE id = ? LIMIT 1');
        if (!$stmt) {
    ...
    public function update(int $id, array $data): ?array
    {
        ...
        $query = 'UPDATE product_attribute_values SET ' . implode(', ', $set) . ' WHERE id = ?';
    ...
    {
        $stmt = $this->db->prepare('DELETE FROM product_attribute_values WHERE id = ?');
        if (!$stmt) {
