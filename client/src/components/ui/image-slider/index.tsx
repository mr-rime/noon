...
export function ImageSlider({
		...
		switch (dotsTheme) {
			...
			case "theme2":
				return (
					<div
						className={cn(
							"group-hover:opacity-100 opacity-0 min-h-[calc(4px * 4)] w-fit px-2 py-[5px] bg-[#ECEDF2] rounded-full transition-opacity duration-300 gap-[4px] flex items-center justify-center absolute bottom-2 left-1/2 -translate-x-1/2",
					...
					>
						{displayImages.map((_, i) => (
							<button
								key={i}
								className={cn(
									`w-[4px] h-[4px] cursor-pointer rounded-full transition-all duration-300 `,
									index === i + 1 ? "bg-[#34353a] w-[12px]" : "bg-[#b6b6b9]",
								)}
								onClick={(e) => {
									e.preventDefault();
									goToSlide(i + 1);
					...
					<div className="min-h-[calc(4px * 4)] w-fit px-2 py-[5px] bg-transparent rounded-full  opacity-[1] visible transition-opacity duration-300 gap-[4px] flex items-center justify-center absolute bottom-5 left-1/2 -translate-x-1/2">
						{displayImages.map((_, i) => (
							<button
								key={i}
								className={cn(
									`w-[6px] h-[6px] cursor-pointer rounded-full transition-all ease-in-out border border-[#404553]`,
	...
	return (
		<div className="w-full h-full group touch-none">
			<div
				className={cn(
					"relative select-none overflow-hidden rounded-[12px]",
					isMobile ? "w-full h-fit" : `overflow-hidden w-full h-[${height}px]`,
				)}
			...
			>
				...
				>
					<div
						ref={containerRef}
						className={cn("flex will-change-transform h-full", !showProductControls && isMobile && "gap-3")}
						style={{
