...
export default function Register({ setForm }: { setForm: React.Dispatch<React.SetStateAction<"login" | "register">> }) {
    ...
    return (
        <form onSubmit={handleSubmit(handleCreatePartner)} className="flex flex-col items-center justify-center bg-white shadow-sm rounded-[4px] w-[80%] max-w-[450px] p-[30px] my-[50px]">
            ...
            <div className="flex flex-col items-center justify-center w-full mt-5">
                <Input type='text' {...register("storeName")} placeholder="Enter your store name" className="w-full " input={{ className: "w-full text-[#404553] focus:border-[#404553] h-[43px]" }} />
            </div>
            <div className="flex flex-col items-center justify-center w-full mt-5">
                <Input type='email' {...register("email")} placeholder="Enter your email" className="w-full " input={{ className: "w-full text-[#404553] focus:border-[#404553] h-[43px]" }} />
            </div>
            <div className="flex flex-col items-center justify-center w-full mt-5">
                <Input type='password' {...register("password")} placeholder="Enter your password" className="w-full " input={{ className: "w-full text-[#404553] focus:border-[#404553] h-[43px]" }} />
            </div>
            <div className="w-full mt-[30px]">
                {
                    loading ? <Button type='button' className="h-[43px] w-full normal-case flex items-center justify-center">
                        <Loader2 className="animate-spin" size={20} />
                    </Button> : <Button type='submit' className="h-[43px] w-full normal-case">Register</Button>
                }
            </div>
            <div className="flex items-center mt-5 space-x-2">
                ...
                {
                    loading ? <button disabled className="text-[#3866df] cursor-pointer">click here</button> : <button onClick={() => {
